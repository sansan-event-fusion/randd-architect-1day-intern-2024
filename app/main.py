...
merged_df = pd.merge(
    merged_df,
    df_cards.loc[:, ["user_id", "position", "company_name"]],
    left_on="owner_user_id",
    right_on="user_id",
    how="left",
)
merged_df = merged_df.rename(columns={"company_name": "owner_company_name", "position": "owner_position"})

# # # APIデータをテーブル形式で表示
# st.subheader("merged_df")
# st.dataframe(merged_df)

df_position_user = merged_df.pivot_table(
    index="owner_company_id", columns="user_position", aggfunc="size", fill_value=0
)
